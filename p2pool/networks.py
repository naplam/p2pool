...
nets = dict(
    ...
    maxcoin=math.Object(
        ...
        SPREAD=30, # blocks
        IDENTIFIER='D0D1D2D3B2F66869'.decode('hex'),
        PREFIX='D0D3D4D541C16869'.decode('hex'),
        P2P_PORT=28668,
        MIN_TARGET=0,
	MAX_TARGET=2**256//2**20 - 1,
        PERSIST=False,
        WORKER_PORT=28660,
        BOOTSTRAP_ADDRS='p2pool.crunchpool.com'.split(' '),
        ANNOUNCE_CHANNEL='#p2pool-max',
        VERSION_CHECK=lambda v: True,
    ),
    maxcoin_testnet=math.Object(
        PARENT=networks.nets['maxcoin_testnet'],
        SHARE_PERIOD=15, # seconds target spacing
        CHAIN_LENGTH=24*60*60//10, # shares
        REAL_CHAIN_LENGTH=24*60*60//10, # shares
        TARGET_LOOKBEHIND=200, # shares coinbase maturity
        SPREAD=30, # blocks
        IDENTIFIER='D0D1D2D3B2F68C69'.decode('hex'),
        PREFIX='D0D3D4D541C11D69'.decode('hex'),
        P2P_PORT=28668,
        MIN_TARGET=0,
        MAX_TARGET=2**256//2**20 - 1,
        PERSIST=False,
