...
def sha3(data):
    return __import__('max_sha3').getPoWHash(data)
...
class Protocol(protocol.Protocol):
    ...
    def sendPacket(self, command, payload2):
        ...
        data = self._message_prefix + struct.pack('<12sI', command, len(payload)) + sha3(payload)[:4] + payload
